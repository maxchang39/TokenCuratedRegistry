...
import "./PLCRDVoting.sol";
import "./ProxyFactory.sol";

contract PLCRDFactory {

  event newPLCR(address creator, EIP20 token, PLCRDVoting plcr);

  ProxyFactory public proxyFactory;
  PLCRDVoting public canonizedPLCR;

  /// @dev constructor deploys a new canonical PLCRVoting contract and a proxyFactory.
  constructor() public {
    canonizedPLCR = new PLCRDVoting();
  ...
  */
  function newPLCRBYOToken(EIP20 _token) public returns (PLCRDVoting) {
    PLCRDVoting plcr = PLCRDVoting(proxyFactory.createProxy(canonizedPLCR, ""));
    plcr.init(_token);
